...
// 🔥 ระบบแยกว่าล็อกอินแล้วจะไปหน้าไหน
onAuthStateChanged(auth, (user) => {
    if (user) {
        const em = user.email.toLowerCase(); 
        const isAdmin = em === "nattezava1996@gmail.com" || em.includes("admin");
        if (isAdmin) {
            window.location.href = 'admin.html'; // เด้งไปหน้า Command Center ของแอดมิน
        } else {
            window.location.href = 'user.html'; // เด้งไปหน้า Workspace ของพนักงาน
        }
    }
